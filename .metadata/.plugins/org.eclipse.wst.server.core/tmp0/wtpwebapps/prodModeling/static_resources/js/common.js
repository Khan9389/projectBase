...
function cf_orgFileNm2Text(target_id, FILE_STOR_NAME){
	var params = {
		FILE_STOR_NAME: FILE_STOR_NAME,
	}
	cf_call("/getFileOrgNm", params, function(data) {$("#" + target_id).val(data.FILE_ORIG_NAME + "." + data.FILE_ORIG_EXT); });
}

function cf_fileDn(FILE_STOR_NAME) {
	var params = {
		FILE_STOR_NAME: FILE_STOR_NAME,
	}
	cf_call("/chkFileExist", params, function(data) { cf_chkFileExistCB(data, FILE_STOR_NAME); });
}
...
$(document).ready(function(){ 
	...
	fileTarget.on('change', function(){ // 값이 변경되면
		console.log($(this))
		var cur = $(this).children(".file-upload").val().substr("12");
		$(this).children(".file-name").val(cur);
		console.log(cur);
  });*/
});